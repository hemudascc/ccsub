...
<header>
    ...
    font-size: 12px;
    padding-left: 20px;">
			<!--   Etisalat: Free for 24 hours, then  AED 11/weekly, VAT including
			 <br>
			...
			<!-- اتصالات: مجانًا لمدة 24 ساعة ، ثم 11 درهمًا إماراتيًا في الأسبوع ، بما في ذلك ضريبة القيمة المضافة
			<br> -->
			 9 دو: درهم إماراتي لكل أسبوع ، شامل ضريبة القيمة المضافة        	
			</div>
			</c:if>
			...
			<p style="text-align:center;"><img src="${pageContext.request.contextPath}/resources/actel/play_it_banner.png" class="img-responsive"
			 ...
			 <form id="operatorSelectForm" method="post" action="${pageContext.request.contextPath}/cnt/actel/tocg">
				<c:if test="${l==0}">
				<p style="text-align:center;font-size:14px;"><strong>Play IT</strong><br/>				
				 Please click on operator button.				   
				   </p>
				   </c:if>
				   <c:if test="${l==1}">
				...
				<br/><br/>
				<button type="submit" class="button" style="font-size: 15px;"> ${l==0?'DU':'دو'}</button><br/><br/>
				<%-- <button type="button" onclick="handelSubmit('52')" class="button" style="font-size: 15px;"> ${l==0?'Etisalat':'اتصالات'}</button><br/><br/> --%>
			 </form>
			</div> 	
			</div>
			</section>
