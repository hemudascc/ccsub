...
public class JMSActelService {
	...
	@Autowired  
	@Qualifier("actelDuDlrJMSTemplate") private JmsTemplate
	actelDuDlrJMSTemplate;
	...
	public boolean saveActelDuDlr(final ActelDuDlr actelDuDlr) { 
		long time=System.currentTimeMillis(); 
		try{
			logger.debug("saveDeliveryReceipt:: ActelDuDlr:: "+actelDuDlr);

			actelDuDlrJMSTemplate.send(new MessageCreator() {

				@Override public Message createMessage(Session session) throws JMSException {
					Message message=session.createObjectMessage(actelDuDlr); return message; }
			});

		}catch(Exception ex){ 
			logger.error("saveDeliveryReceipt::::::::::::: ",ex);
			boolean save=daoService.saveObject(actelDuDlr);
			return save; 
		}
		logger.debug("saveDeliveryReceipt:: total time "+(System.currentTimeMillis()-time));
		return true; 
