...
public enum HongkongSmartoneStatusEnum {
	...
	private static final Logger logger = Logger
			.getLogger(HongkongSmartoneStatusEnum.class);
	private String status;
	private String statusDesc;
	private Boolean retryBilling;
	
	HongkongSmartoneStatusEnum(String status,String statusDesc, Boolean retryBilling){
		this.status=status;
	 ...
	 public static Boolean isRetry(String status){
		 try{
			 for(HongkongSmartoneStatusEnum hongkongSmartoneStatusEnum:values()){
				 if(hongkongSmartoneStatusEnum.getStatus().equalsIgnoreCase(status)){
					 return hongkongSmartoneStatusEnum.getRetryBilling();
				 }
			 }
		 }catch(Exception ex){
